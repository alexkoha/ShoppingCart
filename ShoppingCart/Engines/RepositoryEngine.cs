...
namespace ShoppingCart.Engines
{
    ...
    {
        private static SemaphoreSlim _semaphoreLockSlim;
        ...
        public RepositoryEngine()
        {
            ...
            _locker = new object();
            _semaphoreLockSlim = new SemaphoreSlim(1, 1);
            ...
            Task task = new Task(UpdateXmlFilesAsync);
            task.Start();
        }
        ...
        private async Task UnzipFileAsync(string name, Chain chain)
        {
            await Task.Run(() =>
            {
                ...
                {
                    Debug.WriteLine("Unzip failed :" + exeption.Message);
                }
                finally
                {
                    _semaphoreLockSlim.Release();
        ...
        public IDictionary<Chain ,Tuple<IEnumerable<Product>, IEnumerable<Product>>> GetChipestExpentivestProductsByChains()
        {
            ...
            else
            {
                foreach (var chain in ChainsArchive)
                {
                    ...
                    var orderedCartChain = productOfChain.OrderBy(x => x.ItemPrice).ToList();
